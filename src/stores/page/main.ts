...
export const render = {
  classStatus: (arg: { mainId: string }): string => {
    ...
    arg?.mainId === app.refer.router.query.mainId && classStatus.push(`select`);
  ...
  classLimit: (arg: { mainId: string }): string => {
    ...
    const item = main.state.data[app.refer.router.query.listId].data[arg.mainId];
    const now = new Date();
  ...
  valueCount: (arg: { mainId: string }): string => {
    const itemList = Object.values(sub.state.data[app.refer.router.query.listId].data[arg.mainId].data);
...
export const handle = {
  ...
  inputValue: (arg: { mainId: string, type: `title` | `check` | `task`, value: string | boolean }): void => {
    main.refer.setState((state) => {
      if (arg.type === `title`) {
        state.data[app.refer.router.query.listId].data[arg.mainId][arg.type] = arg.value as string;
      } else if (arg.type === `check` || arg.type === `task`) {
        state.data[app.refer.router.query.listId].data[arg.mainId][arg.type] = arg.value as boolean;
      }
    });
  },
  editItem: (arg?: { mainId: string }): void => {
    main.refer.setState((state) => {
      for (const mainId of state.data[app.refer.router.query.listId].sort) {
    ...
    dialog.handle.open({
      ...
      callback: {
        ok: () => {
          ...
          main.refer.setState((state) => {
            state.data[app.refer.router.query.listId].sort.unshift(`main${id}`);
            state.data[app.refer.router.query.listId].data[`main${id}`] = {
              check: false,
  ...
  copyItem: (arg: { mainId: string }): void => {
    ...
    main.refer.setState((state) => {
      state.data[app.refer.router.query.listId].sort.splice(state.data[app.refer.router.query.listId].sort.indexOf(arg.mainId) + 1, 0, mainId);
      state.data[app.refer.router.query.listId].data[mainId] = lodash.cloneDeep(state.data[app.refer.router.query.listId].data[arg.mainId]);
  ...
  moveItem: (arg: { mainId: string }): void => {
    dialog.handle.open({
      ...
      radio: {
        none: false,
        select: ``,
        sort: list.state.data.sort.filter((listId) => listId !== app.refer.router.query.listId),
      ...
      callback: {
        ok: () => {
          ...
          main.refer.setState((state) => {
            if (listId !== app.refer.router.query.listId) {
              state.data[listId].sort.unshift(arg.mainId);
              state.data[listId].data[arg.mainId] = state.data[app.refer.router.query.listId].data[arg.mainId];
              state.data[app.refer.router.query.listId].sort.splice(state.data[app.refer.router.query.listId].sort.indexOf(arg.mainId), 1);
              delete state.data[app.refer.router.query.listId].data[arg.mainId];
            }
            delete state.status[arg.mainId];
          });
          sub.refer.setState((state) => {
  ...
  dragMove: (arg: { y: number }): void => {
    if (main.refer.drag.status === `move`) {
      ...
      const index = main.state.data[app.refer.router.query.listId].sort.indexOf(main.refer.drag.id!);
      const clone = main.refer.drag.clone!.getBoundingClientRect();
      const prev = app.refer.getById(`MainItem${main.state.data[app.refer.router.query.listId].sort[index - 1]}`)?.getBoundingClientRect();
      const current = app.refer.getById(`MainItem${main.state.data[app.refer.router.query.listId].sort[index]}`).getBoundingClientRect();
      const next = app.refer.getById(`MainItem${main.state.data[app.refer.router.query.listId].sort[index + 1]}`)?.getBoundingClientRect();
      if (prev && clone.top + clone.height / 2 < (next?.top || current.bottom) - (prev.height + current.height) / 2) {
        main.refer.setState((state) => {
          state.data[app.refer.router.query.listId].sort.splice(index - 1, 0, ...state.data[app.refer.router.query.listId].sort.splice(index, 1));
        });
      }
