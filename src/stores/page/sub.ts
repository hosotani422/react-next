...
export const render = {
  valueMemo: (): string => {
    ...
    for (const subId of sub.state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].sort) {
      valueMemo.push(sub.state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].data[subId].title);
    }
  ...
  classLimit: (): string => {
    ...
    const item = main.state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId];
    const now = new Date();
  ...
  textAlarm: (): string => {
    ...
    for (const alarmId of main.state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].alarm) {
...
export const handle = {
  ...
  inputValue: (arg: { subId: string, type: `title` | `check`, value: string | boolean }): void => {
    sub.refer.setState((state) => {
      if (arg.type === `title`) {
        state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].data[arg.subId][arg.type] = arg.value as string;
      } else if (arg.type === `check`) {
        state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].data[arg.subId][arg.type] = arg.value as boolean;
      }
    });
  },
  inputMemo: (arg: { value: string }): void => {
    ...
    sub.refer.setState((state) => {
      state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].sort = [];
      state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].data = {};
      for (const [i, title] of arg.value.split(`\n`).entries()) {
        state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].sort.push(`sub${subId + i}`);
        state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].data[`sub${subId + i}`] = { check: false, title };
      }
    });
  },
  divideItem: async (arg: { subId: string, caret: number }): Promise<void> => {
    const subId = `sub${new Date().valueOf()}`;
    const title = sub.state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].data[arg.subId].title;
    await sub.refer.setState((state) => {
      state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].sort.splice(sub.state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].sort.indexOf(arg.subId) + 1, 0, subId);
      state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].data[arg.subId].title = title.slice(0, arg.caret);
      state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].data[subId] = { check: false, title: title.slice(arg.caret) };
    });
  ...
  connectItem: async (arg: { subId: string }): Promise<void> => {
    const subId = sub.state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].sort[sub.state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].sort.indexOf(arg.subId) - 1]!;
    const caret = sub.state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].data[subId].title.length;
    await sub.refer.setState((state) => {
      state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].sort.splice(state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].sort.indexOf(arg.subId), 1);
      state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].data[subId].title += state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].data[arg.subId].title;
      delete state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].data[arg.subId];
  ...
  openCalendar: (): void => {
    calendar.handle.open({
      select: main.state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].date,
      cancel: app.refer.i18next.t(`button.cancel`),
      clear: app.refer.i18next.t(`button.clear`),
      callback: (date) => {
        main.refer.setState((state) => {
          state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].date = date;
        });
  ...
  openClock: (): void => {
    clock.handle.open({
      time: datefns.format(`2000/1/1 ${main.state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].time || `00:00`}`, `HH:mm`),
      cancel: app.refer.i18next.t(`button.cancel`),
      clear: app.refer.i18next.t(`button.clear`),
      ok: app.refer.i18next.t(`button.ok`),
      callback: (args) => {
        main.refer.setState((state) => {
          state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].time = args ? datefns.format(`2000/1/1 ${args.hour}:${args.minute}`, `HH:mm`) : ``;
        });
        clock.handle.close();
      },
    });
  ...
  openAlarm: (): void => {
    dialog.handle.open({
      ...
      check: {
        ...
        data: (() => {
          ...
          for (const id of app.refer.i18next.t(`dialog.alarm.sort`, { returnObjects: true })) {
            data[id] = {
              check: main.state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].alarm.includes(id),
      ...
      callback: {
        ok: () => {
          main.refer.setState((state) => {
            state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].alarm = [];
            for (const id of dialog.state.check.sort) {
              if (dialog.state.check.data[id].check) {
                state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].alarm.push(id);
              }
            }
          });
          dialog.handle.close();
  ...
  dragMove: (arg: { y: number }): void => {
    if (sub.refer.drag.status === `move`) {
      ...
      const index = sub.state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].sort.indexOf(sub.refer.drag.id!);
      const clone = sub.refer.drag.clone!.getBoundingClientRect();
      const wrap = app.refer.getById(`SubBody`).getBoundingClientRect();
      const prev = app.refer.getById(`SubItem${sub.state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].sort[index - 1]}`)?.getBoundingClientRect();
      const current = app.refer.getById(`SubItem${sub.state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].sort[index]}`).getBoundingClientRect();
      const next = app.refer.getById(`SubItem${sub.state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].sort[index + 1]}`)?.getBoundingClientRect();
      if (prev && clone.top + clone.height / 2 < (next?.top || wrap.bottom) - (prev.height + current.height) / 2) {
        sub.refer.setState((state) => {
          state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].sort.splice(index - 1, 0, ...state.data[app.refer.router.query.listId].data[app.refer.router.query.mainId].sort.splice(index, 1));
        });
      }
