...
export const handle = {
  setup: (): void => {
    ...
    dialog.handle.setup();
    notice.handle.setup();
    app.refer.router = NextRouter.useRouter();
    app.refer.i18next = reactI18next.useTranslation();
  },
  init: async (): Promise<void> => {
    if (app.refer.init && typeof window !== `undefined`) {
  ...
  routerList: (): void => {
    app.refer.router.push(`/${app.refer.router.query.listId}/list`);
  },
  routerMain: (arg: { listId: string }): void => {
    app.refer.router.push(`/${arg.listId}`);
  },
  routerSub: (arg: { mainId: string }): void => {
    app.refer.router.push(`/${app.refer.router.query.listId}/sub/${arg.mainId}`);
  },
  routerConf: (): void => {
    app.refer.router.push(`/${app.refer.router.query.listId}/conf`);
  },
  routerBack: (): void => {
    app.refer.router.back();
  },
  clearTrash: (): void => {
